...
import { formatLtv, formatReadableDecimalNumber, Token, formatBalanceWithSymbol, abbreviateAddress } from "@morpho-org/uikit/lib/utils";
...
function StatsGrid({
  ...
  const userCollateral = position 
    ? Number(formatUnits(position.collateral, collateralToken.decimals ?? 18)) * collateralTokenPriceInUSD
    : 0;
  ...
  return (
    <div className="mb-8 grid grid-cols-4 gap-6">
      ...
      <div>
        <p className="text-muted-foreground mb-1 text-sm">Max LTV</p>
        <p className="text-2xl font-semibold">{formatReadableDecimalNumber({ value: lltv, maxDecimals: 2 })}%</p>
      </div>
      <div>
        <p className="text-muted-foreground mb-1 text-sm">Your Position</p>
        <p className="text-2xl font-semibold">
          ${formatReadableDecimalNumber({ value: userCollateral + userBorrow, maxDecimals: 2, letters: true })}
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="mb-8">
      ...
      <p className="text-muted-foreground text-sm leading-relaxed">
        Supply {collateralToken.symbol} as collateral to borrow {loanToken.symbol}. Monitor your health factor to avoid liquidation.
      </p>
    </div>
  );
}
...
function MarketDetailsGrid({
  ...
  const utilizationRate = market.totalSupplyAssets > 0n 
    ? Number(formatUnits(market.totalBorrowAssets * parseUnits("1", 18) / market.totalSupplyAssets, 18)) * 100
    : 0;

  return (
    <div className="mb-8 grid grid-cols-3 gap-6">
      <div className="border-border rounded-lg border bg-white p-6 shadow-sm">
        <p className="text-muted-foreground mb-2 text-sm">Oracle</p>
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm">{abbreviateAddress(market.params.oracle)}</span>
          {chain?.blockExplorers?.default.url && (
            <a
              href={`${chain.blockExplorers.default.url}/address/${market.params.oracle}`}
      ...
      <div className="border-border rounded-lg border bg-white p-6 shadow-sm">
        <p className="text-muted-foreground mb-2 text-sm">Utilization Rate</p>
        <p className="font-semibold">{formatReadableDecimalNumber({ value: utilizationRate, maxDecimals: 2 })}%</p>
      </div>
      <div className="border-border rounded-lg border bg-white p-6 shadow-sm">
        <p className="text-muted-foreground mb-2 text-sm">IRM</p>
        <div className="flex items-center gap-2">
          <span className="font-mono text-sm">{abbreviateAddress(market.params.irm)}</span>
          {chain?.blockExplorers?.default.url && (
            <a
              href={`${chain.blockExplorers.default.url}/address/${market.params.irm}`}
              rel="noopener noreferrer"
              target="_blank"
            >
              <ExternalLink className="h-4 w-4" />
      ...
      <div>
        <div className="border-border bg-muted/50 grid grid-cols-4 gap-4 border-b p-4 text-sm font-medium">
          <div className="flex items-center gap-1">
            Vault <ChevronDown className="h-4 w-4" />
          </div>
          <div className="flex items-center gap-1">
            Total Supply <ChevronDown className="h-4 w-4" />
          </div>
          <div className="flex items-center gap-1">
            Curators <ChevronDown className="h-4 w-4" />
          </div>
          <div></div>
        </div>
        ...
        <div>
          {marketVaults.length > 0 ? (
            marketVaults.map((vault, index) => (
              <div key={index} className="grid grid-cols-4 items-center gap-4 border-b p-4">
                <div className="flex items-center gap-2">
                ...
                <div className="flex justify-end">
                  <Link
                    to={`/${chainSlug}/vault/${vault.address}`}
            ...
            <p className="p-4 text-center text-muted-foreground">No vaults found for this market</p>
          )}
        </div>
      </div>
    </div>
...
function InteractionSection({
  ...
  const inputValue = currentToken.decimals !== undefined ? parseUnits(textInputValue, currentToken.decimals) : undefined;
  ...
  const supplyCollateralTxnConfig =
    selectedTab === Actions.SupplyCollateral && 
    userAddress !== undefined && 
    inputValue !== undefined
      ? ({
  ...
  const withdrawCollateralTxnConfig =
    selectedTab === Actions.WithdrawCollateral && 
    userAddress !== undefined && 
    inputValue !== undefined
      ? ({
          address: morpho?.address ?? "0x",
  ...
  const healthFactor = position?.ltv !== undefined && market.params.lltv > 0n 
  ...
  return (
    <div className="flex flex-col gap-6">
            ...
            <TabsContent key={action} value={action}>
              <div className={STYLE_INPUT_WRAPPER}>
                ...
                <TokenAmountInput
                  ...
                  usdPrice={[Actions.SupplyCollateral, Actions.WithdrawCollateral].includes(selectedTab) 
                    ? collateralTokenPriceInUSD 
                    : loanTokenPriceInUSD}
                />
              </div>
      ...
      <div className="border-border rounded-lg border bg-white shadow-sm">
        ...
        <div className="border-border w-full rounded-lg border-t bg-white p-4 space-y-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground text-sm">
                (${formatReadableDecimalNumber({ 
                  value: parseFloat(userCollateralBalance) * collateralTokenPriceInUSD, 
                  maxDecimals: 2 
                })})
              </span>
            </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground text-sm">
                (${formatReadableDecimalNumber({ 
                  value: parseFloat(userBorrowBalance) * loanTokenPriceInUSD, 
                  maxDecimals: 2 
                })})
              </span>
            </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className={`font-medium ${healthFactor < 20 ? 'text-red-500' : healthFactor < 50 ? 'text-yellow-500' : 'text-green-500'}`}>
...
export function MarketSubPage() {
  ...
  const { borrowMarketsArray: markets, marketVaults } = useVaults({ 
    chainId, 
  ...
  const rewards = borrowingRewards.get(marketId as any) ?? [];
  ...
  const { data: positionRaw } = useReadContract({
    ...
    args: userAddress && marketId ? [marketId, userAddress] : undefined,
  ...
  const position = useMemo(() => {
    ...
    return new AccrualPosition(
      { user: userAddress!, supplyShares, borrowShares, collateral },
      currentMarket
    );
      ...
      <div className="flex h-full grow justify-center pb-16">
        <div className="mx-auto flex max-w-7xl gap-10 px-5 py-3">
          <div className="w-8/12">
            ...
            />
            <VaultsSection
              marketVaults={currentMarketVaults}
              loanToken={loanToken}
              chain={chain}
            />
          </div>
